...
The registry includes `weather.lookup` and `device.status`. `weather.lookup` is **IMPLEMENTED** through a dedicated server-side Google Weather API adapter; `device.status` remains a deterministic placeholder. The weather boundary is `weather.lookup → validated latitude/longitude → weather adapter → Google Weather API current conditions → normalized JUNI result`. The adapter uses the fixed official `weather.googleapis.com/v1/currentConditions:lookup` endpoint with metric units, a bounded timeout and response size, redirect rejection, and safe provider/network error normalization. It accepts coordinates only: no geocoding or address/name lookup is implemented.

`GOOGLE_WEATHER_API_KEY` is a server-only environment value. It is never included in tool metadata, logs, client storage, or normalized results; the official API's required query-key mechanism is used only inside the server adapter. The normalized DTO contains location, observation time, timezone, condition, Celsius temperatures, humidity percentage, wind speed in meters per second, and precipitation probability when present. Raw provider responses, headers, authorization material, and provider error bodies are not returned. The tool remains authenticated, `read_only`, confirmation-free, and server-only.

Weather alerts, forecast endpoints, generic web browsing, arbitrary HTTP tools, browser automation, WhatsApp or messaging, desktop automation, scheduling, shell execution, code execution, and model-driven automatic tool execution remain **NOT IMPLEMENTED**. The registry is intentionally not exposed as a browser-facing tRPC endpoint in this foundation slice, and no confirmation UI has been added.

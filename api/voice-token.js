...
export default async function handler(req, res) {
  ...
  const rate = checkRateLimit(clientKey(req) + ":voice-token", Number.isFinite(limit) && limit > 0 ? limit : 10, Number.isFinite(windowSeconds) && windowSeconds > 0 ? windowSeconds : 60);
  ...
  if (!app.config.voice.enabled) return json(res, 503, { error: "Realtime voice is disabled.", code: "VOICE_FEATURE_DISABLED", requestId });
  ...
  try {
    ...
    return json(res, 503, { error: error?.message || "Voice identity is not configured.", code: error?.code || "VOICE_AUTH_REQUIRED", requestId });
  }

  const body = req.body && typeof req.body === "object" ? req.body : {};
  const resumeHandle = typeof body.resumeHandle === "string" && body.resumeHandle.length <= 8192 ? body.resumeHandle : null;
  const requestedSessionId = typeof body.sessionId === "string" && body.sessionId.length <= 128 ? body.sessionId : null;
  ...
  try {
    ...
    app.events.emit("voice.token.requested", { sessionId, resumed: Boolean(resumeHandle) }, { requestId, provider: "gemini", model: token.model });
    ...
    console.error("JUNI-AI voice token request failed", { code: error?.code, name: error?.name, message: error?.message, requestId });
    const code = error?.code === "VOICE_MODEL_UNSUPPORTED" ? "VOICE_MODEL_UNSUPPORTED" : "VOICE_TOKEN_FAILED";
    return json(res, error?.code === "VOICE_MODEL_UNSUPPORTED" ? 503 : 502, {
      error: code === "VOICE_MODEL_UNSUPPORTED" ? "The configured Gemini Live model is not supported for realtime voice." : "A realtime voice token could not be created.",
      code,
      requestId,
    });
  }
